class SevenErrors  extends Instruction {
    ...
    removeStimuli(stimuli){
        ...
        if (typeof stimuli === 'string' || stimuli instanceof String){
   ...
   terminate(){
    ...
    update(dt){
        if(!this.activity.isRunning())
            return;
        if(this.foundErrorsCount == this.errors.length){ //encontrou todos
            this.terminate();
        }
    ...
    pointerUp(evt){
        ...
        for (var i = 0; i< this.errors.length; i++){            
            ...
            if(this.errors[i].wasPointed()){
                ...
                if(!this.errors[i].found){
                    this.errors[i].setFound();
                    this.foundErrorsCount++;
                }                
            }            
        }
    }
